...
document.getElementById('nextOtp').addEventListener('click',function(){
    if(tester == 1){
        ...
        vehicletype = localStorage.getItem('y');
        latitude=localStorage.getItem('Latitude');
        longi=localStorage.getItem('Longitude');
        tcontact=localStorage.getItem('tcontact');
        opentime=localStorage.getItem('opentime');
        closetime=localStorage.getItem('closetime');
        mechsessionemail=localStorage.getItem('mechsessionemail');
        alert(mechsessionemail);
        //alert(longi);
        $.ajax({
           
             type:'POST',
           data:{fname: fname, lname: lname,address: address, contact: contact, sname: sname, servicetype: servicetype, vehicletype: vehicletype,mechsessionemail: mechsessionemail, latitude: latitude, longitude: longi, tcontact: tcontact, opentime: opentime, closetime: closetime},
           url:'http://localhost/wheelcare/addshop.php',
           
             success:function(result){
                 alert(result);
                if(result=="true")
                {
                    window.location.href="shopDiv.html";
                }
            }
        });
    }else{
